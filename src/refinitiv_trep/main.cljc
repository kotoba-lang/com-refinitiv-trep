(ns refinitiv-trep.main
  "Kotodama WASM entrypoint for the Refinitiv_trep clean-room actor (L4) — Clojure port.
  ...
  facts (`refinitiv_trep.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "refinitiv_trep")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument"  :plural "instruments"  :id-prefix "refiniti_ins"
    :fields [:symbol :assetClass :exchange :currency]  :required [:symbol :assetClass]
    :coerce {}                                          :refs {}}
   {:entity "Quote"       :plural "quotes"       :id-prefix "refiniti_quo"
    :fields [:instrumentId :bid :ask :asOf]            :required [:bid :ask]
    :coerce {:bid :float :ask :float}                  :refs {:instrumentId "Instrument"}}
   {:entity "Order"       :plural "orders"       :id-prefix "refiniti_ord"
    :fields [:instrumentId :side :quantity :price :status] :required [:side :quantity]
    :coerce {:quantity :float :price :float}           :refs {:instrumentId "Instrument"}}
   {:entity "Execution"   :plural "executions"   :id-prefix "refiniti_exe"
    :fields [:orderId :quantity :price :executedAt]   :required [:quantity :price]
    :coerce {:quantity :float :price :float}           :refs {:orderId "Order"}}
   {:entity "MarketData"  :plural "marketdatas"  :id-prefix "refiniti_mar"
    :fields [:instrumentId :open :high :low :close]   :required [:open :high]
    :coerce {:open :float :high :float :low :float :close :float} :refs {:instrumentId "Instrument"}}
   {:entity "Position"    :plural "positions"    :id-prefix "refiniti_pos"
    :fields [:instrumentId :quantity :avgPrice]       :required [:quantity :avgPrice]
    :coerce {:quantity :float :avgPrice :float}        :refs {:instrumentId "Instrument"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"refinitiv_trep.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "refinitiv_trep-compat" :tier tier :entities entities} 200])
...
(defn start! [] :refinitiv_trep-compat/ready)
